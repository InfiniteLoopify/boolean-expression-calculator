		...
		int convertPostfix()
		{
		    ...
		    for(int i = 0; i < m_str.length(); ++i)
		    {
		        ...
		        else if(getPriority(m_str[i]) >= 2)
		        {
					...
					//if operator is 2 charactered (like == or !=) then store as single character
...
//print the operators allowed to use in expression
void printRules()
{
	std::cout << "(+)  Addition\n"
				 "(-)  Subtraction\n"
...
int main()
{
    ...
    //if the expression is valid then display, else show invalid expression
